...
import nyullog.blog.config.jwt.JwtFactory;
...
class TokenApiControllerTest {
    ...
    @DisplayName("createNewAccessToken: 새로운 엑서스 토큰을 발급한다.")
    @Test
    public void createNewAccessToken() throws Exception{
        ...
        final String requstBody = objectMapper.writeValueAsString(request);

        //when
        ResultActions resultActions = mockMvc.perform(post(url).contentType(MediaType.APPLICATION_JSON_VALUE).content(requstBody));

        //then
        resultActions.andExpect(status().isCreated()).andExpect(jsonPath("$.accessToekn").isNotEmpty());
    }
